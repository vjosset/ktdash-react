...
export default function Faction(props) {
  ...
  const cards2021 = faction.killteams
    ?.filter((killteam) => killteam.edition === "kt21")
    ?.map((killteam) => (
      <Card
      ...
      >
        <Stack>
          <Image
            ...
            radius="sm"
            src={`${
              !killteam.isCustom ? "https://ktdash.app" : ""
            }/img/portraits/${faction.factionid}/${killteam.killteamid}/${
              killteam.killteamid
            }.jpg`}
          />
          <Title order={3}>
            {!!killteam.isCustom && (
              <Popover width={200} position="bottom" withArrow shadow="md">
                <Popover.Target>
                  <ActionIcon mr={5} variant="outline">
                    <IconFlask />
                  </ActionIcon>
                </Popover.Target>
                <Popover.Dropdown>
          ...
          <span
            dangerouslySetInnerHTML={{
              __html: `${killteam.description.split("<br/>")[0]}`,
            }}
          />
        </Stack>
      </Card>
    ));
  const cards2024 = faction.killteams
          ...
          >
            {!!killteam.isCustom && (
              <Popover width={200} withArrow shadow="md">
                ...
                <Popover.Dropdown>
                  <Text>
                    This is experimental content and should not be used in
                    competitive events.
                  </Text>
                </Popover.Dropdown>
              </Popover>
      ...
      <Stack>
        <SimpleGrid my="md" cols={{ base: 1, sm: 2 }} spacing="md">
          <Image
            ...
            h={300}
            radius="sm"
            src={`https://ktdash.app/img/portraits/${faction?.factionid}/${faction?.factionid}.jpg`}
          />
        ...
        <Tabs defaultValue={settings.edition || "kt24"}>
          <Stack>
            <Tabs.List grow>
              {(!settings.edition || settings.edition === "kt21") && (
                <Tabs.Tab value="kt21">KT2021</Tabs.Tab>
              )}
              {(!settings.edition || settings.edition === "kt24") && (
                <Tabs.Tab value="kt24">KT2024</Tabs.Tab>
              )}
            </Tabs.List>
